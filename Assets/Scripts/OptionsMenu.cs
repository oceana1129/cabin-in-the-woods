...
public class OptionsMenu : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (PlayerPrefs.HasKey("opt_volume"))
            initialVolume = PlayerPrefs.GetFloat("opt_volume");
        else
        {
            initialVolume = 0.5f;
    ...
    private void SyncAspectDropdownState()
    {
        ...
        if (aspectRatioDropdown)
            aspectRatioDropdown.interactable = windowed;
    }

    private void BuildAspectRatiosTMP()
    {
        ...
        foreach (var r in Screen.resolutions)
        {
            ...
            if (!exists)
                aspects.Add(new Aspect { w = rw, h = rh, ratio = ar, label = $"{rw}:{rh}" });
        }
